import { Configuration, OpenAIApi } from "openai";

const configuration = new Configuration({
  apiKey: process.env.OPENAI_API_KEY,
});
const openai = new OpenAIApi(configuration);

export default async function (req, res) {
  const { occasion, priceMin, priceMax, gender, age, hobbies } = req.body;
  const prompt = generatePrompt(occasion, priceMin, priceMax, gender, age, hobbies);

  // console.log(prompt);

  if (!configuration.apiKey) {
    res.status(500).json({
      error: {
        message: "OpenAI API key not configured, please follow instructions in README.md",
      }
    });
    return;
  }

  try {
    const completion = await openai.createCompletion({
      model: "text-davinci-003",
      prompt: prompt,
      temperature: 0.6,
      max_tokens: 2048,
    });
    const result = completion.data.choices[0].text.split('\n');
    // console.log(completion.data.choices[0].text);
    // console.log(result);
    res.status(200).json({ result: result });
  } catch (error) {
    // Consider adjusting the error handling logic for your use case
    if (error.response) {
      console.error(error.response.status, error.response.data);
      res.status(error.response.status).json(error.response.data);
    } else {
      console.error(`Error with OpenAI API request: ${error.message}`);
      res.status(500).json({
        error: {
          message: 'An error occurred during your request.',
        }
      });
    }
  }
}

function generatePrompt(occasion, priceMin, priceMax, gender, age, hobbies) {
  return `Suggest three gift ideas for a ${occasion} present with prices between ${priceMin}
and ${priceMax} dollars for a ${age} year old ${gender} that is into ${hobbies}.

presents:`;
}
